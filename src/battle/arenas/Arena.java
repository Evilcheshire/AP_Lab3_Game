...
public class Arena {
    ...
    private boolean isValidPosition(int x, int y, GameObject object) {
        return x >= 0 && x < WIDTH && y >= 0 && y < HEIGHT && (object.getX() - x <= 4 && object.getY() - y <= 4) && !grid[x][y].isGivenType(GameObjectTypes.DROID);
    }
    ...
    public void placeObject(int y, int x, GameObject object) {
        if (isValidPosition(x, y, object)) {
            ...
            System.out.println(" Coordinates ( " + x + "; " + y + " ) are already taken by " + object.getName());
    }
    ...
    public void showArena(){
        for (int i = 0; i < HEIGHT; i++) {
            ...
            for (int j = 0; j < WIDTH; j++) {
                if (grid[i][j] != null && (grid[i][j].isGivenType(GameObjectTypes.DROID) && grid[i][j].isAlive()))
